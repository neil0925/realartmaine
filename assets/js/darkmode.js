...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const applyMode = () => {
    ...
    // sync the new theme-switch checkbox if present
    try {
      const cb = document.querySelector('.theme-switch__checkbox');
      if (cb) cb.checked = !!isDark;
    } catch (e) {}
  };

  // Ensure the Uiverse theme-switch control exists inside the dropdown
  function ensureThemeSwitchExists() {
    if (!dropdown) return;
    if (dropdown.querySelector('.theme-switch')) return;
    ...
    const cb = dropdown.querySelector('.theme-switch__checkbox');
    if (cb) {
      try { cb.checked = safeGet('darkMode') === 'true'; } catch (e) {}
  ...
  if (gear && dropdown) {
    ...
    gear.addEventListener("click", (ev) => {
      ...
      // ensure the custom theme switch is present when dropdown opens
      if (isShown) ensureThemeSwitchExists();
    });
  ...
  // ensure the theme-switch exists (in case dropdown never opened yet)
  try { ensureThemeSwitchExists(); } catch (e) {}
