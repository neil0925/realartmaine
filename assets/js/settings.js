// settings.js
// Wires the gear dropdown and provides a single image-cache clear action.
(function () {
  ...
  async function clearImageCacheOnly() {
    ...
    try { await caches.delete(IMAGE_CACHE); } catch (e) {}
    try { localStorage.removeItem(IMAGE_REVALIDATION_STATE_KEY); } catch (e) {}
  ...
  function createSettingsUI() {
    ...
    clearCacheBtn.addEventListener('click', async function () {
      ...
      try {
        ...
        updateButtonTemporarily(clearCacheBtn, 'Cleared ' + removed + ' cached images', 2600);
      } catch (err) {
        alert(err && err.message ? err.message : 'Failed to clear image cache.');
      }
    });
