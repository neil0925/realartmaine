...
// Try to load a generated images.json from site root (preferred, same-origin)
...
// On DOM ready: prefer images.json (produced by CI), then GitHub API, otherwise use embedded static imagesList
document.addEventListener("DOMContentLoaded", async () => {
  // try site-generated images.json first
  let imgs = await fetchImagesJson();
  if (!imgs) {
    // try GitHub Contents API as a fallback (works for public repos)
    const fetched = await fetchImagesFromGitHub();
    if (Array.isArray(fetched) && fetched.length > 0) imgs = fetched;
  }
  // if we obtained an array, use it; otherwise keep the embedded imagesList (already present in this file)
